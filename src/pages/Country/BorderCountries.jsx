...
export default function BorderCountries({ countries }) {
  ...
  if (countries.includes("ISR")) {
    countries = countries.splice(countries.indexOf("ISR"), 1, "PSE");
  }
  return (
        ...
        {countries.map((border) => {
          ...
          return (
            <Link
              ...
              className="rounded px-5 py-1 text-sm capitalize shadow-xl dark:bg-blue dark:shadow-lg sm:text-base focus-visible:outline focus-visible:outline-darkBlue dark:focus-visible:outline-white "
              key={border}
            >
              {countryName}
            </Link>
